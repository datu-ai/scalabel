...
export function makeMockGRPCStub (
  ...
  stub.createDeploymentTask = jest.fn().mockImplementation((
    ...
    callback: (
      error: Error | null, result: messages.CreateDeploymentTaskResponse
    ) => void) => {
  ...
  stub.deployModel = jest.fn().mockImplementation((
    ...
    callback: (
      error: Error | null, result: messages.DeployResponse
    ) => void) => {
  ...
  stub.performInference = jest.fn().mockImplementation((
    request: messages.InferenceRequest,
    callback: (
      error: Error | null, result: messages.InferenceResponse) => void
  ) => {
